...
bootstrap_f <- function(nbase){
  ...
  for(i in 1:50){
    ...
    for(j in 1:length(xlogregrun$y)){
      norm.int <- boot.ci(bootstrap.V.norisk, type = c("norm"), index = j)
      ci.norisk.norm.low[j] <- norm.int$normal[2]
      ci.norisk.norm.up[j] <- norm.int$normal[3]
    }
    A[i, ] <- ci.norisk.norm.low
    B[i, ] <- ci.norisk.norm.up
  }
  ci <- list(A, B)
  return(ci)
}
