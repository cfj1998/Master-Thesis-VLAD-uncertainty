...
vlad_ci <- function(nbase){
  ...
  for(i in 1:50){
    ...
    xlogregrun$y <- rbinom(1000, size = 1, prob = exp(xbetarun)/(1+exp(xbetarun)))
    regmod <- glm(y~1, data <- xlogregbase, family = binomial("logit"))
    pred <- predict(regmod, newdata = xlogregrun, se.fit = TRUE)
    p.fit <- inverse_logit(pred$fit)
    p.low <- inverse_logit(pred$fit-1.96*pred$se.fit)
    p.high <- inverse_logit(pred$fit+1.96*pred$se.fit)
    V.min <- cumsum(p.low-xlogregrun$y)
    V.max <- cumsum(p.high-xlogregrun$y)
    A[i, ] <- V.min
    B[i, ] <- V.max
  }
  ci <- list(A, B)
