...
vlad_ci <- function(nbase){
  ...
  for(i in 1:50){
    ...
    xlogregrun$y <- rbinom(1000, size = 1, prob = exp(xbetarun)/(1+exp(xbetarun)))
    regmod <- glm(y~1, data = xlogregbase, family = binomial("logit"))
    p_0 <- mean(xlogregbase$y)
    regmodrun <- glm(y~1, data = xlogregrun, family = binomial("logit"))
    t <- seq(from=1, to = length(xlogregrun$y))
    p_1 <- cumsum(xlogregrun$y)/t
    n_0 <- length(xlogregbase$y)
    V.nonrisk.mean <- cumsum(p_0-p_1)
    V.norisk.var <- cumsum(p_1*(1-p_1))
    V.norisk.se <- sqrt(V.norisk.var)
    V <- numeric(0)
    V[1] <- p_0 - xlogregrun$y[1]
    for(j in 2:1000){
      V[j] <- V[j-1] + (p_0 - xlogregrun$y[j])
    }
    dim(V) <- c(1000, 1)
...
vlad_ci <- function(nbase){
  ...
  for(i in 1:50){
    ...
    n_0 <- length(xlogregbase$y)
    V.nonrisk.mean <- cumsum(p_0-p_1)
    V.norisk.var <- ((t^2)/(n_0))*p_0*(1-p_0)+cumsum(p_0*(1-p_0))
    V.norisk.se <- sqrt(V.norisk.var)
    V <- numeric(0)
    V[1] <- p_0 - xlogregrun$y[1]
    for(j in 2:1000){
