...
#MSE
t <- seq(from = 1, to = 25841, by = 1)
average_V_t.nonrisk <- (1/t)*cumsum(V.nonrisk)
MSE.nonrisk <- vlad_norisk.var +(average_V_t.nonrisk-V.nonrisk)^2
par(mfrow = c(1, 2))
plot(MSE.nonrisk)
average_V_t.risk <- (1/t)*cumsum(V.risk)
MSE.risk <- vlad_risk.var +(average_V_t.risk-V.risk)^2
plot(MSE.risk)

#Delta_V
delta_v.nonrisk <- yhat-rundata1$y
delta_v.risk <- p_0.risk-rundata1$y
par(mfrow = c(1, 2))
plot(delta_v.nonrisk)
plot(delta_v.risk)
