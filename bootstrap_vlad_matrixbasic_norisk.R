...
bootstrap_f <- function(nbase){
  ...
  for(i in 1:50){
    ...
    for(j in 1:length(xlogregrun$y)){
      basic.int <- boot.ci(bootstrap.V.norisk, type = c("basic"), index = j)
      ci.norisk.basic.low[j] <- basic.int$basic[4]
      ci.norisk.basic.up[j] <- basic.int$basic[5]
    }
    A[i, ] <- ci.norisk.basic.low
    B[i, ] <- ci.norisk.basic.up
  }
  ci <- list(A, B)
  return(ci)
}
...
abline(h = 0, lty = 2)
matplot(t(lower_limit2), type = "l", col = "red", lwd = 1, ylim = c(-300, 300), ylab = "V_t",
