...
vlad_ci <- function(nbase){
  ...
  for(i in 1:50){
    ...
    xlogregrun$y <- rbinom(1000, size = 1, prob = exp(xbetarun)/(1+exp(xbetarun)))
    regmod <- glm(y~x1+x2+x3, data = xlogregbase, family = binomial("logit"))
    p_0 <- predict(regmod, newdata = xlogregrun, type = "response")
    p_0.average <- mean(p_0)
    regmodrun <- glm(y~x1+x2+x3, data = xlogregrun, family = binomial("logit"))
    t <- seq(from=1, to = length(xlogregrun$y))
    p_1 <- predict(regmodrun, newdata = xlogregrun, type = "response")
    n_0 <- length(xlogregbase$y)
    V.nonrisk.mean <- cumsum(p_0-p_1)
    V.norisk.var <- cumsum(p_1*(1-p_1))
    V.norisk.se <- sqrt(V.norisk.var)
    V <- numeric(0)
    V[1] <- p_0[1] - xlogregrun$y[1]
    for(j in 2:1000){
      V[j] <- V[j-1] + (p_0[j] - xlogregrun$y[j])
    }
    dim(V) <- c(1000, 1)
    V.min <- V - 1.96*V.norisk.se
    V.max <- V + 1.96*V.norisk.se
    A[i, ] <- V.min
    B[i, ] <- V.max
  }
  ci <- list(A, B)
...
vlad_ci <- function(nbase){
  ...
  for(i in 1:50){
    ...
    n_0 <- length(xlogregbase$y)
    V.nonrisk.mean <- cumsum(p_0-p_1)
    V.norisk.var <- ((t^2)/(n_0))*p_0.average*(1-p_0.average)+cumsum(p_0.average*(1-p_0.average))
    V.norisk.se <- sqrt(V.norisk.var)
    V <- numeric(0)
    V[1] <- p_0[1] - xlogregrun$y[1]
    for(j in 2:1000){
