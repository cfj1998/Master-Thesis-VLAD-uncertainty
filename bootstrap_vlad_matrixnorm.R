...
bootstrap_f <- function(nbase){
  ...
  for(i in 1:50){
    ...
    xlogregrun$y <- rbinom(1000, size = 1, prob = exp(xbetarun)/(1+exp(xbetarun)))
    regmod <- glm(y~x1+x2+x3, data <- xlogregbase, family = binomial("logit"))
    bootstrap.V.risk <- boot(xlogregbase, V.risk, R = 2000, rundata = xlogregrun)
    ci.risk.norm.low <- numeric(0)
    ci.risk.norm.up <- numeric(0)
    for(j in 1:length(xlogregrun$y)){
      norm.int <- boot.ci(bootstrap.V.risk, type = c("norm"), index = j)
      ci.risk.norm.low[j] <- norm.int$normal[2]
      ci.risk.norm.up[j] <- norm.int$normal[3]
    }
    A[i, ] <- ci.risk.norm.low
    B[i, ] <- ci.risk.norm.up
  }
  ci <- list(A, B)
  return(ci)
...
matlines(t(vlad3), col = "black", lty = 1, lwd = 2)
abline(h = 0, lty = 2)
