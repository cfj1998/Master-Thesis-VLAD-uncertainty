...
bootstrap_f <- function(nbase){
  ...
  for(i in 1:50){
    ...
    for(j in 1:length(xlogregrun$y)){
      basic.int <- boot.ci(bootstrap.V.risk, type = c("basic"), index = j)
      ci.risk.basic.low[j] <- basic.int$basic[4]
      ci.risk.basic.up[j] <- basic.int$basic[5]
    }
    A[i, ] <- ci.risk.basic.low
    B[i, ] <- ci.risk.basic.up
  }
  ci <- list(A, B)
  return(ci)
}
